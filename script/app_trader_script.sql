...
SELECT 
CASE
    WHEN a1.primary_genre = LEFT(g1.genres, POSITION(';' IN g1.genres) - 1) THEN a1.primary_genre
    ELSE LEFT(g1.genres, POSITION(';' IN g1.genres) - 1)
END as genre, 
ROUND(AVG((a1.rating+g1.rating)/2),2) AS avg_rating
FROM play_store_apps as g1
INNER JOIN app_store_apps as a1 USING(name)
WHERE POSITION(';' IN g1.genres) > 0
GROUP BY genre
ORDER BY avg_rating DESC;
-- Top genres are Action(4.50), Racing(4.50), Adventure(4.46), Card(4.45), and Puzzle(4.44) .
